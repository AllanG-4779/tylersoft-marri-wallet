...
CREATE TYPE event_time_display AS ENUM (
    'start_and_end',
    'start_only'
    );

CREATE TYPE ticket_tier_status AS ENUM (
    'upcoming',
    'active',
    'ended',
    'sold_out',
...
-- ============================================================
-- 2. CATEGORIES  (self-referencing, 2-level max)
-- ============================================================
-- parent_id IS NULL  → category    e.g. "Music"
-- parent_id NOT NULL → subcategory e.g. "Concert"
...
(
    ...
    online_event_url        TEXT, -- for online / hybrid
    ...
    time_display            event_time_display NOT NULL DEFAULT 'start_and_end',
...
-- Event managers (staff assigned to manage or operate an event)
...
(
    ...
    CONSTRAINT chk_tt_sold_lte_capacity
        CHECK (quantity_sold <= total_capacity),
    CONSTRAINT chk_group_capacity
        CHECK (
            (is_group_ticket = FALSE) OR
            (is_group_ticket = TRUE AND total_capacity % group_size = 0)
            ),
    CONSTRAINT chk_group_size
        CHECK (
...
(
    ...
    status         ticket_tier_status NOT NULL DEFAULT 'upcoming',
    sort_order     INTEGER            NOT NULL DEFAULT 0,
    created_at     TIMESTAMPTZ        NOT NULL DEFAULT NOW(),
    updated_at     TIMESTAMPTZ        NOT NULL DEFAULT NOW(),
...
-- ============================================================
-- 11. ATTENDEES  (one row = one ticket / QR code)
-- ============================================================
...
-- speeds up the "last scan per attendee" lookup used at the gate
...
BEGIN
    NEW.status = CASE
                     WHEN NEW.quantity_sold >= NEW.quantity THEN 'sold_out'::ticket_tier_status
                     WHEN NOW() < NEW.starts_at THEN 'upcoming'::ticket_tier_status
                     WHEN NOW() BETWEEN NEW.starts_at
                         AND NEW.ends_at THEN 'active'::ticket_tier_status
                     ELSE 'ended'::ticket_tier_status
        END;
    RETURN NEW;
END;
$$;
...
CREATE TRIGGER trg_pricing_tier_status
    ...
    ON pricing_tiers
    FOR EACH ROW
EXECUTE FUNCTION fn_pricing_tier_status();
...
BEGIN
    IF NEW.status = 'completed' AND OLD.status IS DISTINCT FROM 'completed' THEN
        ...
        FROM (SELECT pricing_tier_id, SUM(quantity) AS qty
              FROM order_items
              WHERE order_id = NEW.id
                AND pricing_tier_id IS NOT NULL
...
CREATE TRIGGER trg_order_complete_sync
    ...
    ON orders
    FOR EACH ROW
EXECUTE FUNCTION fn_sync_on_order_complete();
...
-- Event KPIs (Overview dashboard tab)
...
-- Active pricing tier per ticket type (used at checkout)
CREATE OR REPLACE VIEW vw_active_pricing AS
SELECT DISTINCT ON (ticket_type_id) ticket_type_id,
                                    id                       AS pricing_tier_id,
                                    name                     AS tier_name,
                                    price,
...
-- Gate status per attendee (used by check-in service before allowing entry)
-- Resolves the re-entry abuse case:
--   last_scan_status = 'checked_in'  → still inside, REJECT re-entry
--   last_scan_status = 'checked_out' → legitimately left, ALLOW re-entry
--   last_scan_status = NULL          → never scanned, ALLOW first entry
...
SELECT a.id                 AS attendee_id,
       ...
       last_scan.created_at AS last_scan_at,
       last_scan.staff_id   AS last_scanned_by,
       -- single flag the gate app checks before allowing entry
       CASE
           WHEN a.is_cancelled THEN FALSE -- ticket cancelled
           WHEN last_scan.status IS NULL THEN TRUE -- never scanned, allow
           WHEN last_scan.status = 'checked_out' THEN TRUE -- properly exited, allow
           WHEN last_scan.status = 'checked_in' THEN FALSE -- still inside, reject
           ELSE FALSE
           END              AS can_enter
FROM attendees a
         LEFT JOIN LATERAL (
    SELECT status, created_at, staff_id
    FROM check_in_logs
...
SELECT a.id            AS attendee_id,
       ...
       a.checked_in_at,
       a.check_in_count,
       a.checked_in_by AS checked_in_by_user_id
FROM attendees a
...
-- ============================================================
-- Tables (13):
--   event_categories          self-referencing (category + subcategory)
--   tags
--   events                    venue fields live directly here
--   event_tags                M2M
--   event_media
--   event_managers            owner / manager / staff per event
--   ticket_types
--   pricing_tiers
--   promo_codes
--   orders
